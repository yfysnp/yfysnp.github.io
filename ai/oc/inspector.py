...
def wake_message_for(event):
    ...
    ⚠️ 措辞必须**把范围锁死在"被打断的那一件事"**，不能给开放式指令。
    实测（2026-09-04 一次真实唤起）原文案"请继续处理未完成的事情"没有界定范围，
    数字人把它当成"盘点一遍所有悬着的线头"，回过来的是"当前有 2 项待你决定"——
    连历史上早就搁置的非主线任务一起翻出来排查。唤起的本意只是"你上一轮被异常
    打断了，把那一轮接着做完"，扩大到全量待办是净负担：既浪费一轮算力，也可能
    真的动到不该动的东西。

    所以文案统一遵守三条：
      1. 指明**唯一**要接的那件事（上一轮的任务 / 某个 sub 的回报 / 用户某条消息）
      2. 用"只 ⋯"这个限定词收范围，不写"不要盘点其他历史待办"那类否定式尾句 ——
         同样的意思说两遍太啰嗦，而且提负面指令反而把"其他待办"摆到了它眼前
      3. 不替它决定怎么做（是追问还是重试由它自己判断，巡检器不懂业务）
    """
    detail = event.get("detail") or {}
    reason = {
        "ALERT_MODEL_ERROR":
            f"上一轮因模型调用异常（{detail.get('reason', '未知')}）中断，"
            f"请只把那一轮被打断的任务接着做完",
        "ALERT_CC_STALLED":
            "当前的代码执行子任务长时间无响应，请只确认这一个子任务的状态并接着推进",
        "ALERT_SUB_NOT_REPORTED":
            f"你派给 {'、'.join(detail.get('subs') or ['基础 Agent'])} 的那个任务还没有回报，"
            f"请只跟进这一项",
        "ALERT_USER_NOT_REPLIED":
            "用户最近这条消息似乎没有得到回复，请只回复这一条",
        "ALERT_DA_NOT_REPLIED_AFTER_SUB":
            f"{detail.get('sub', '基础 Agent')} 刚回报的结果还没有转达给用户，"
            f"请只转达这一条结果",
        "ALERT_SUB_RUN_FAILED":
...
def check_user_not_replied(human, group_id, sessions, cfg, now_ms, transcript):
    ...
    if swallowed_gap > threshold(cfg, "MSG_SWALLOWED_MS"):
        return {
            ...
            #
            # 尾句给的是**一个能绕开这条坏链路的动作**：侧边栏直接读会话本身、不经过
            # 群消息投递，所以即使群里这条消息丢了，那边也看得到数字人的真实进展。
            # 原来写的"建议换个方式确认"太含糊 —— 用户不知道"换个方式"是哪个方式。
